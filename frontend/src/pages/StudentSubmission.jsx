    ...
    <div style={S.body}>
      ...
      <div style={S.wrapper}>
        ...
        <div style={S.pageHeader}>
          <h1 style={S.h1}>My Assignments &amp; Submissions</h1>
          <p style={S.sub}>View, submit, and track your assignments</p>
        </div>
                  ...
                  {assignments.map((asgn) => {
                    ...
                    return (
                      <tr key={asgn.id}>
                        ...
                        <td style={S.td}>
                          {asgn.due_date ? new Date(asgn.due_date).toLocaleDateString() : "---"}
                        </td>
        ...
        {selectedAsgn && !alreadySubmitted(selectedAsgn.id) && (
          <div style={S.card}>
            <div style={S.cardTitle}>📤 Submit: {selectedAsgn.title}</div>

            <div style={{ background: "#f8faff", border: "1px solid #e0e8f8", borderRadius: 10, padding: 16, marginBottom: 18 }}>
              <div style={{ fontSize: 13, color: "#1a1f2e", marginBottom: 6 }}>
                <strong>Instructions:</strong>{" "}{selectedAsgn.description || "No instructions provided."}
              </div>
            ...
            {isLate(selectedAsgn.due_date) && (
              <div style={{ background: "#fee2e2", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#991b1b", fontWeight: 600, marginBottom: 14 }}>
            ...
            <div style={S.formRow}>
              <div style={{ ...S.formGroup, flex: 2 }}>
                ...
                <div
                  style={{ ...S.uploadArea, borderColor: file ? "#22b14c" : "#c5d0e8" }}
                ...
                {file && (
                  <button style={{ ...S.btn, ...S.btnDanger, ...S.btnSm, marginTop: 6, alignSelf: "flex-start" }} onClick={() => setFile(null)}>
                    Remove File
                  </button>
                )}
              </div>
            ...
            <div style={S.formGroup}>
              <label style={S.label}>Or Enter Text</label>
              <textarea
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 8, flexWrap: "wrap" }}>
              <button style={{ ...S.btn, ...S.btnSuccess }} onClick={handleSubmit} disabled={loading}>
                {loading ? "Submitting..." : "✅ Submit Assignment"}
              </button>
              <button style={{ ...S.btn, ...S.btnOutline }} onClick={() => setSelectedAsgn(null)}>
                Cancel
              </button>
            </div>
          </div>
                ...
                <tbody>
                  {mySubmissions.map((sub) => (
                    <tr key={sub.id}>
                      ...
                      <td style={S.td}>
                        {sub.file ? (
                          <a href={`http://127.0.0.1:8000${sub.file}`} target="_blank" rel="noreferrer" style={{ color: "#3d6ff5", fontSize: 12 }}>
                            ⬇ Download
                          </a>
                        ) : sub.url_entry ? (
                      ...
                      <td style={S.td}>
                        <StatusBadge status={sub.marks !== undefined && sub.marks !== null ? "evaluated" : "pending"} />
                      </td>
                      <td style={S.td}>
                        {sub.marks !== undefined && sub.marks !== null ? (
                      ...
                      <td style={S.td}>
                        {sub.feedback ? (
                          <div style={{ background: "#f8faff", border: "1px solid #e0e8f8", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#333", maxWidth: 260 }}>
                            {sub.feedback}
                          </div>
                        ) : (
                          <span style={{ color: "#8892a4", fontSize: 12 }}>Awaiting feedback</span>
                        )}
                      </td>
                    </tr>
                  ))}
